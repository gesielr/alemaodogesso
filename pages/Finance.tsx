...
const Finance: React.FC<FinanceProps> = ({ filterType }) => {
  ...
  // Filter transactions based on prop
  const displayedTransactions = filterType 
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
           <button className="flex items-center bg-white border border-gray-300 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
      ...
      {/* Stats Cards - Only show relevant ones based on filter */}
          ...
          {(!filterType || filterType === TransactionType.RECEITA) && (
            <div className="bg-green-50 p-6 rounded-xl border border-green-100">
              ...
              <p className="text-3xl font-bold text-green-800">
                  {formatMoney(transactions.filter(t => t.type === TransactionType.RECEITA).reduce((acc, curr) => acc + (curr.paid_amount || 0), 0))}
              </p>
              <p className="text-xs text-green-600 mt-1">
                  Pendente: {formatMoney(transactions.filter(t => t.type === TransactionType.RECEITA).reduce((acc, curr) => acc + (curr.amount - (curr.paid_amount || 0)), 0))}
              </p>
            </div>
          )}
          ...
          {(!filterType || filterType === TransactionType.DESPESA) && (
            <div className="bg-red-50 p-6 rounded-xl border border-red-100">
              ...
              <p className="text-3xl font-bold text-red-800">
                  {formatMoney(transactions.filter(t => t.type === TransactionType.DESPESA).reduce((acc, curr) => acc + (curr.paid_amount || 0), 0))}
              </p>
               <p className="text-xs text-red-600 mt-1">
                  Pendente: {formatMoney(transactions.filter(t => t.type === TransactionType.DESPESA).reduce((acc, curr) => acc + (curr.amount - (curr.paid_amount || 0)), 0))}
              </p>
            </div>
          )}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 font-semibold text-gray-800 flex justify-between items-center">
          ...
          <span className="text-xs font-normal text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
            {displayedTransactions.length} registros
          </span>
        </div>
                 ...
                 <div className="pt-4 flex justify-end gap-3">
                    ...
                    <button 
                        ...
                        className={`px-4 py-2 text-white rounded-lg font-medium flex items-center ${
                            selectedTx.type === TransactionType.RECEITA ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
                        }`}
                    >
